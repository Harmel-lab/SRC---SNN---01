...
--
-- Last Modification Date : may 12th, 2026
--
...
use work.Src_neuro_L04;

package NetWorkLevel04_pkg is
    component NetWorkLevel04
        Port (
            -- Reset --
            ...
            MuR_in      : in std_logic_vector (0 to 5);                    -- largueur du bus de données muMachine
            MuR_out     : out std_logic_vector (0 to 5);                   -- largueur du bus de données muMachine
        
            -- Data Level Input-Output
            Spike_I		: IN std_logic_vector (ML04Y-1   downto 0);
            Spike_O		: out std_logic_vector (ML04X-1   downto 0)
            );
    end component;
end NetWorkLevel04_pkg;

------------------------------------------------------------------
-- Definition des interconnexions -> NetWorkLevel04       		--
------------------------------------------------------------------
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use IEEE.NUMERIC_STD.ALL;
...
architecture NetWorkLevel04_Full of NetWorkLevel04 is
    ...
    signal LMuR_in    : std_logic_vector (0 to 5);
    signal Lreset     : std_logic :='1';
    signal LGo        : std_logic :='0';
    signal LReady     : std_logic_vector (ML04X-1   downto 0) := (others => '0');
    signal LSpike_I   : std_logic_vector (ML04Y-1   downto 0) := (others => '0');
    signal LSpike_O   : std_logic_vector (ML04X-1   downto 0) := (others => '0');
 ...
 begin   
	...
	-------------------------------------------------------------  
	u1 : for i in 0 to (ML04X-1) generate
                    SRC_one : entity Src_neuro_L04 port map( 
                    ...
                    MuR_in      => LMuR_in(0 to 1),             -- transmission micro-machine
	...
	------------------------------------------------  
    process(CLK, Reset, NmaC, MuR_in(1))
    begin 
