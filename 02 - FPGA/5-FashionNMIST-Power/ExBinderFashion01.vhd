...
entity ExbinderFashion is
     Port (
        ...
        -- IO Système --
        hub75_R1    : OUT std_logic;
        hub75_G1    : OUT std_logic;
        hub75_B1    : OUT std_logic;
        hub75_R2    : OUT std_logic       
    );
end ExbinderFashion;
...
architecture ExbinderF_Full of ExbinderFashion is
   ...
   --------------------------------------------------------------
	-- Horloge system  - active la liaison package CLOCK       --
	-------------------------------------------------------------  
   ibufds1_inst : IBUFDS
        generic map (
          DIFF_TERM => TRUE,
	...
	------------------------------------------------  
    Myval <= std_logic_vector(to_unsigned(L6L7, 4));
    hub75_R1 <= Myval(0);
    hub75_G1 <= Myval(1); 
    hub75_B1 <= Myval(2);
    hub75_R2 <= Myval(3);
