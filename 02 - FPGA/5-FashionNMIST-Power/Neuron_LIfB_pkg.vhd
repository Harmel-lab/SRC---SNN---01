...
package LInt_neuro_pkg is
	component LInt_neuro
		port (
            ...
            NumberCell      : integer range 0 to ML05X-1;                       -- numéro de cellule 
            Spike_I		    : IN std_logic_vector (ML05Y-1   downto 0);         -- all input spikes
            Spike_O 	    : OUT integer range -2000 to +2000 := 0   		    -- Output spike pulse
 		 );
	end component;
end LInt_neuro_pkg;
...
entity LInt_neuro is
	port (
        ...
        NumberCell      : integer range 0 to ML05X-1;                       -- numéro de cellule 
        Spike_I		    : IN std_logic_vector (ML05Y-1   downto 0);         -- all input spikes
        Spike_O 	    : OUT integer range -20000 to +20000 := 0   	    -- Output spike pulse -> valeur de l'intégration
         );
...
architecture LInt_neuro_all of LInt_neuro is 
	...
	signal tmpcurSUP  : integer range -2048 to +2047 :=0;
	...
	constant ML05       : framebuffer_ML05 :=ML05;                      -- matrice de poids 

    signal icur         : integer range 0 to ML05Y := 0;                -- variable de boucle somme courant
	                            ...
	                            when 0 =>
	                                if icur<ML05Y then
	                                    if (Spike_I(icur)='1') then
	                                        if ML05(NumberCell,icur) ='1' then
	                                            tmpcur := tmpcur + ML05Val1;
	                                        else
	                                            tmpcur := tmpcur + ML05Val0;
	                                        end if;
	                                    end if;      
	                                  tmpcurSUP <= tmpcur;
	                                  icur<= icur+1;
